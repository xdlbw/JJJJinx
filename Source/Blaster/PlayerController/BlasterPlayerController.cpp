...
void ABlasterPlayerController::SetHUDMacthCountDown(float CountDownTime)
{
	BlasterHUD = BlasterHUD == nullptr ? Cast<ABlasterHUD>(GetHUD()) : BlasterHUD;
	bool bHUDValid = BlasterHUD &&
		BlasterHUD->CharacterOverlay &&
		BlasterHUD->CharacterOverlay->MacthCountDownText;
	if (bHUDValid) {
		if (CountDownTime < 0.f) {
			BlasterHUD->CharacterOverlay->MacthCountDownText->SetText(FText());
			return;
		}
		...
		BlasterHUD->CharacterOverlay->MacthCountDownText->SetText(FText::FromString(CountDownText));
	}
}
...
void ABlasterPlayerController::HandleCooldown()
{
	...
	if (BlasterHUD) {
		...
		if (bHUDValid) {
			...
			FString AnnouncementText = Announcement::NewMacthStartsIn;
